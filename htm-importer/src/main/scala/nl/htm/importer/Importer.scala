...
case class EventData(arenas: Int, participants: List[Participant], tournaments: List[Tournament], subscriptions: Map[Tournament, List[Participant]])
...
object DummyImporter extends Importer[EmptySettings] {
  override def doImport(s: EmptySettings = EmptySettings()): EventData = {
    ...
    EventData(2, ps, ts, Map(ts.map(t => t -> (ps filter (_ => r.nextDouble < 0.75))): _*))
  }
}